...
# 通知所有的线程frame 数据更新了
def notifySubThread(frame):
    print('开始通知需要frame的线程')
    for username, thread in threads.items():
        thread.frame = frame
        print('已通知{}'.format(username))

# 通知客户端
def notify(message):
    print('开始返回页面数据')
    msg_bytes = stringTobytesForChinese(message)
    #msg_bytes = stringTobytes(message)
    #msg_bytes = binaryTobytes(message)
    #msg_bytes = message
    for connection in clients.values():
        connection.send(msg_bytes)

def binaryTobytes(binary_data):
    # 发送二进制数据
    ...
    if binary_len <= 125:
        backMsgList.append(struct.pack('b', binary_len))
    elif binary_len <= 65535:
        backMsgList.append(struct.pack('b', 126))
        backMsgList.append(struct.pack('>i', binary_len))
    elif binary_len <= (2 ^ 64 - 1):
...
def stringTobytesForChinese(data):
...
class websocket_thread(threading.Thread):
    def __init__(self, connection, username, frame=None):
